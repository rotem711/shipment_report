...
const main = () => {
  ...
  xlData.map((item) => {
    for (let i = 0; i < ACCOUNT_IDENTIFIERS.length; i++) {
      for (const key in LOOKUP_KEYS) {
        for (let j = 0; j < LOOKUP_KEYS[key].length; j++) {
          ...
          if (item[ACCOUNT_IDENTIFIERS[i]].toLowerCase().indexOf(lookupKey.toLowerCase()) > -1) {
    ...
    rowsByAccount[item["Senders Name"]] = rowsByAccount[item["Senders Name"]] || []
  ...
  const transactionReportData = newData.map((item) => ({
    ...
    "Customer Total Extra Charges": item["Customer Total Extra Charges"].toFixed(2),
  ...
  writeCSV(`${OUTPUT_PATH}/${todayFolderPath}/transaction-report_${todayFolderPath}.csv`, headers, transactionReportData)

  writeCSV(`${OUTPUT_PATH}/${todayFolderPath}/summary_${todayFolderPath}.csv`, headers, summaryDataNested)
}
